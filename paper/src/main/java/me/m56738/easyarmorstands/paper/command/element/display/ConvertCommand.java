package me.m56738.easyarmorstands.paper.command.element.display;
...
import me.m56738.easyarmorstands.platform.paper.entity.PaperArmorStand;
...
public class ConvertCommand {
    ...
    public void convert(
        ...
        ElementType itemDisplayType = elementTypeRegistry.get(EntityTypeKeys.ITEM_DISPLAY);
        ...
        for (Element element : selection.elements()) {
            ...
            ArmorStand entity = PaperArmorStand.toNative(((ArmorStandElement) element).getEntity());
            ...
            convert(eas, sender, entity, equipment.getHelmet(), ArmorStandPart.HEAD, ItemDisplay.ItemDisplayTransform.HEAD, headMatrix, actions, createdElements, itemDisplayType);
            convert(eas, sender, entity, equipment.getItemInMainHand(), ArmorStandPart.RIGHT_ARM, ItemDisplay.ItemDisplayTransform.THIRDPERSON_RIGHTHAND, rightMatrix, actions, createdElements, itemDisplayType);
            convert(eas, sender, entity, equipment.getItemInOffHand(), ArmorStandPart.LEFT_ARM, ItemDisplay.ItemDisplayTransform.THIRDPERSON_LEFTHAND, leftMatrix, actions, createdElements, itemDisplayType);
            if (!actions.isEmpty()) {
    ...
    private void convert(EasyArmorStandsCommon eas, EasPlayer sender, ArmorStand entity, ItemStack item, ArmorStandPart part, ItemDisplay.ItemDisplayTransform itemTransform, Matrix4dc matrix, List<Action> actions, List<Element> elements, ElementType itemDisplayType) {
        ...
        Vector3d offset = info.getOffset(ArmorStandSize.get(entity.isSmall()), 1).rotateY(Util.getRoundedYawAngle(location.getYaw()), new Vector3d());
        location.add(offset.x, offset.y, offset.z);

        Rotations rotations = part.getPose(PaperArmorStand.fromNative(entity));
        Matrix4d transform = new Matrix4d()
                .rotateY(Util.getRoundedYawAngle(location.getYaw()))
                .rotateZYX(-Math.toRadians(rotations.z()), -Math.toRadians(rotations.y()), Math.toRadians(rotations.x()))
                .mul(matrix);
        ...
        properties.put(EntityPropertyTypes.LOCATION, PaperAdapter.fromNative(location));
        properties.put(ItemDisplayPropertyTypes.ITEM, PaperItemStack.fromNative(item));
        properties.put(ItemDisplayPropertyTypes.TRANSFORM, PaperAdapter.fromNative(itemTransform));
        ...
        actions.add(new ElementCreateAction(eas, element));
