...
public class OptionalTextDialogEntry implements DialogEntry {
    ...
    public void populateInputs(Collection<DialogInput> inputs, Locale locale) {
        String text = MiniMessage.miniMessage().serialize(property.getValue().orElse(Component.empty()));
    ...
    public void save(DialogResponseView response) {
        ...
        if (text != null) {
            if (text.isEmpty()) {
                ...
                property.setValue(Optional.of(MiniMessage.miniMessage().deserialize(text)));
            }
        }
    }
