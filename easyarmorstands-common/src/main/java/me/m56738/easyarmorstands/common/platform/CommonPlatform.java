...
public interface CommonPlatform extends Platform {
    Configuration getConfiguration();

    GizmoFactory getGizmoFactory(Player player);

    BlockData createBlockData(String input);
    ...
    EntityType getItemDisplayType();

    EntityType getTextDisplayType();

    EntityType getInteractionType();

    void onStartSession(Session session);

    void onStopSession(Session session);

    void onElementInitialize(DefaultEntityElement element);
