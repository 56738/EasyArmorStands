...
import me.m56738.easyarmorstands.common.editor.node.DisplayBoxNode;
import me.m56738.easyarmorstands.common.editor.node.DisplayNode;
import me.m56738.easyarmorstands.common.editor.node.DisplayShearNode;
...
public class DisplayCommands {
    ...
    public void setBlockBrightness(PlayerCommandSource source, ElementSelection selection, EasyArmorStandsCommon eas,
        ...
        try (ManagedChangeContext context = eas.getChangeContextFactory().create(sender)) {
            ...
            Brightness brightness = property.getValue()
                    .orElseGet(() -> location.getBlock().getBrightness())
                    .withBlock(blockLight);
            if (property.setValue(Optional.of(brightness))) {
                sender.sendMessage(Message.success("easyarmorstands.success.changed-brightness",
                        property.getType().getValueComponent(Optional.of(brightness))));
            } else {
                sender.sendMessage(Message.error("easyarmorstands.error.cannot-change"));
            }
        }
    ...
    @RequireElementSelection
    public void convert(
            PlayerCommandSource source, Session session, ElementSelection selection, EasyArmorStandsCommon eas) {
        Player sender = source.source();
        ...
        for (Element element : selection.elements()) {
            ...
            convert(sender, armorStandElement, size, EquipmentSlot.HEAD, ArmorStandPart.HEAD, ItemDisplayTransform.HEAD, headMatrix, actions, createdElements, eas);
            convert(sender, armorStandElement, size, EquipmentSlot.HAND, ArmorStandPart.RIGHT_ARM, ItemDisplayTransform.THIRDPERSON_RIGHTHAND, rightMatrix, actions, createdElements, eas);
            convert(sender, armorStandElement, size, EquipmentSlot.OFF_HAND, ArmorStandPart.LEFT_ARM, ItemDisplayTransform.THIRDPERSON_LEFTHAND, leftMatrix, actions, createdElements, eas);
            if (!actions.isEmpty()) {
        ...
        if (groupRootNode != null) {
            ...
            if (selectionNode != null) {
                selectionNode.selectElements(createdElements.stream()
                        .filter(e -> e instanceof SelectableElement)
                        .map(e -> (SelectableElement) e)
                        .toList());
            }
        }
    }
    ...
    private void convert(Player sender, ArmorStandElement element, ArmorStandSize size, EquipmentSlot slot, ArmorStandPart part, ItemDisplayTransform itemTransform, Matrix4dc matrix, List<Action> actions, List<Element> elements, EasyArmorStandsCommon eas) {
        ...
        Element newElement = type.createElement(properties);
        if (newElement == null) {
            return;
        }

        actions.add(new ElementCreateAction(eas, newElement));
