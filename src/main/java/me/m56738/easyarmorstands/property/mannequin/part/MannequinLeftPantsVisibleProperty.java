package me.m56738.easyarmorstands.property.mannequin.part;

import com.destroystokyo.paper.SkinParts;
import me.m56738.easyarmorstands.api.SkinPart;
import me.m56738.easyarmorstands.api.property.type.MannequinPropertyTypes;
import org.bukkit.entity.Mannequin;

public class MannequinLeftPantsVisibleProperty extends AbstractMannequinSkinPartVisibleProperty {
    public MannequinLeftPantsVisibleProperty(Mannequin mannequin) {
        super(mannequin, MannequinPropertyTypes.SKIN_PART_VISIBLE.get(SkinPart.LEFT_PANTS));
    }

    @Override
    protected boolean getValue(SkinParts parts) {
        return parts.hasLeftPantsEnabled();
    }

    @Override
    protected void setValue(SkinParts.Mutable parts, boolean value) {
        parts.setLeftPantsEnabled(value);
    }
}
